var dictionary = {
    ...
    "force.remove.host.warning": "Warning: Choosing this option will cause Cosmic to forcefully stop all running virtual machines before removing this host from the cluster.",
    ...
    "label.add.affinity.group": "Add new anti-affinity group",
    "label.add.by": "Add by",
    ...
    "label.add.private.network": "Add private network",
    ...
    "label.associated.network.short": "Assoc. Network",
    ...
    "label.change.affinity": "Change Anti-Affinity",
    ...
    "label.created.by": "Created by user",
    ...
    "label.delete.affinity.group": "Delete Anti-Affinity Group",
    ...
    "label.ha.workers.list": "HA Workers List",
    ...
    "label.health.check.message.desc": "Your load balancer will automatically perform health checks on your Cosmic instances and only route traffic to instances that pass the health check",
    ...
    "label.installWizard.subtitle": "This tour will aid you in setting up your Cosmic installation",
    "label.installWizard.title": "Hello and Welcome to Cosmic",
    ...
    "label.introduction.to.cloudstack": "Introduction to Cosmic",
    ...
    "label.ldap.authentication.enabled": "LDAP Authentication",
    "label.ldap.linkedou": "LDAP linked OU for auto provisioning",
    "label.ldap.not.linked": "LDAP OU not linked",
    ...
    "label.ldap.type": "LDAP Account Type",
    "label.level": "Level",
    ...
    "label.previous.state": "Previous State",
    ...
    "label.restart.vpc": "Restart VPC with Cleanup",
    ...
    "label.route.table": "Route table",
    "label.statistics": "Statistics",
    "label.status": "Status",
    "label.step": "Step",
    "label.step.taken": "Step Taken",
    "label.step.1": "Step 1",
    ...
    "label.virtual.machine.id": "Virtual Machine ID",
    "label.virtual.machine.name": "Virtual Machine Name",
    "label.virtual.machine.state": "Virtual Machine State",
    ...
    "label.vpn.users": "VPN Remote Access Users",
    ...
    "label.whohasthismac": "Who has this MAC?",
    ...
    "label.zoneWizard.trafficType.management": "Management: Traffic between Cosmic's internal resources, including any components that communicate with the Management Server, such as hosts and Cosmic system VMs",
    ...
    "message.action.vmsnapshot.create": "Please confirm that you want to take a snapshot of this instance. <br>Please notice that by default a <em>live</em> snapshot is going to be taken. If you want the VM to be paused during the snapshot (and resumed afterwards), please check \"Quiesce\" mode.",
    ...
    "message.delete.affinity.group": "Please confirm that you would like to remove this anti-affinity group.",
    ...
    "message.desc.create.ssh.key.pair": "Please fill in the following data to create or register a ssh key pair.<br><br>(1) If public key is set, Cosmic will register the public key. You can use it through your private key.<br><br>(2) If public key is not set, Cosmic will create a new SSH Key pair. In this case, please copy and save the private key. Cosmic will not keep it.<br>",
    "message.desc.created.ssh.key.pair": "Created a SSH Key Pair.",
    "message.desc.host": "Each cluster must contain at least one host (computer) for guest VMs to run on, and we will add the first host now. For a host to function in Cosmic, you must install hypervisor software on the host, assign an IP address to the host, and ensure the host is connected to the Cosmic management server.<br/><br/>Give the host's DNS or IP address, the user name (usually root) and password, and any labels you use to categorize hosts.",
    ...
    "message.desc.zone": "A zone is the largest organizational unit in Cosmic, and it typically corresponds to a single datacenter. Zones provide physical isolation and redundancy. A zone consists of one or more pods (each of which contains hosts and primary storage servers) and a secondary storage server which is shared by all pods in the zone.",
    ...
    "message.guest.traffic.in.basic.zone": "Guest network traffic is communication between end-user virtual machines. Specify a range of IP addresses that Cosmic can assign to guest VMs. Make sure this range does not overlap the reserved system IP range.",
    ...
    "message.installWizard.copy.whatIsACluster": "A cluster provides a way to group hosts. The hosts in a cluster all have identical hardware, run the same hypervisor, are on the same subnet, and access the same shared storage. Virtual machine instances (VMs) can be live-migrated from one host to another within the same cluster, without interrupting service to the user. A cluster is the third-largest organizational unit within a Cosmic deployment. Clusters are contained within pods, and pods are contained within zones.<br/><br/>Cosmic allows multiple clusters in a cloud deployment, but for a Basic Installation, we only need one cluster.",
    "message.installWizard.copy.whatIsAHost": "A host is a single computer. Hosts provide the computing resources that run the guest virtual machines. Each host has hypervisor software installed on it to manage the guest VMs (except for bare metal hosts, which are a special case discussed in the Advanced Installation Guide). For example, a Linux KVM-enabled server, a Citrix XenServer server, and an ESXi server are hosts. In a Basic Installation, we use a single host running XenServer or KVM.<br/><br/>The host is the smallest organizational unit within a Cosmic deployment. Hosts are contained within clusters, clusters are contained within pods, and pods are contained within zones.",
    ...
    "message.installWizard.copy.whatIsAZone": "A zone is the largest organizational unit within a Cosmic deployment. A zone typically corresponds to a single datacenter, although it is permissible to have multiple zones in a datacenter. The benefit of organizing infrastructure into zones is to provide physical isolation and redundancy. For example, each zone can have its own power supply and network uplink, and the zones can be widely separated geographically (though this is not required).",
    "message.installWizard.copy.whatIsCloudStack": "Cosmic&#8482 is a software platform that pools computing resources to build public, private, and hybrid Infrastructure as a Service (IaaS) clouds. Cosmic&#8482 manages the network, storage, and compute nodes that make up a cloud infrastructure. Use Cosmic&#8482 to deploy, manage, and configure cloud computing environments.<br/><br/>Extending beyond individual virtual machine images running on commodity hardware, Cosmic&#8482 provides a turnkey cloud infrastructure software stack for delivering virtual datacenters as a service - delivering all of the essential components to build, deploy, and manage multi-tier and multi-tenant cloud applications. Both open-source and Premium versions are available, with the open-source version offering nearly identical features.",
    "message.installWizard.copy.whatIsCosmic": "Cosmic is a software platform that pools computing resources to build public, private, and hybrid Infrastructure as a Service (IaaS) clouds. Cosmic manages the network, storage, and compute nodes that make up a cloud infrastructure. Use Cosmic to deploy, manage, and configure cloud computing environments.<br/><br/>Extending beyond individual virtual machine images running on commodity hardware, Cosmic provides a turnkey cloud infrastructure software stack for delivering virtual datacenters as a service - delivering all of the essential components to build, deploy, and manage multi-tier and multi-tenant cloud applications. Both open-source and Premium versions are available, with the open-source version offering nearly identical features.",
    "message.installWizard.copy.whatIsPrimaryStorage": "A Cosmic cloud infrastructure makes use of two types of storage: primary storage and secondary storage. Both of these can be iSCSI or NFS servers, or localdisk.<br/><br/><strong>Primary storage</strong> is associated with a cluster, and it stores the disk volumes of each guest VM for all the VMs running on hosts in that cluster. The primary storage server is typically located close to the hosts.",
    ...
    "message.installWizard.tooltip.addCluster.name": "A name for the cluster.  This can be text of your choosing and is not used by Cosmic.",
    ...
    "message.installWizard.tooltip.addPod.reservedSystemEndIp": "This is the IP range in the private network that the Cosmic uses to manage Secondary Storage VMs and Console Proxy VMs. These IP addresses are taken from the same subnet as computing servers.",
    ...
    "message.installWizard.tooltip.addPod.reservedSystemStartIp": "This is the IP range in the private network that the Cosmic uses to manage Secondary Storage VMs and Console Proxy VMs. These IP addresses are taken from the same subnet as computing servers.",
    ...
    "message.no.affinity.groups": "You do not have any anti-affinity groups. Please continue to the next step.",
    ...
    "message.public.traffic.in.advanced.zone": "Public traffic is generated when VMs in the cloud access the internet. Publicly-accessible IPs must be allocated for this purpose. End users can use the Cosmic UI to acquire these IPs to implement NAT between their guest network and their public network.<br/><br/>Provide at least one range of IP addresses for internet traffic.",
    ...
    "message.select.affinity.groups": "Please select any anti-affinity groups you want this VM to belong to:",
    ...
    "message.storage.traffic": "Traffic between Cosmic's internal resources, including any components that communicate with the Management Server, such as hosts and Cosmic system VMs. Please configure storage traffic here.",
    ...
    "message.tooltip.internal.dns.1": "Name of a DNS server for use by Cosmic internal system VMs in the zone. The private IP address for the pods must have a route to this server.",
    "message.tooltip.internal.dns.2": "Name of a DNS server for use by Cosmic internal system VMs in the zone. The private IP address for the pods must have a route to this server.",
    ...
    "message.your.cloudstack.is.ready": "Your Cosmic is ready!",
